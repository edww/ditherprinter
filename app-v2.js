...
function blur(src,w,h,r){let a=src,b=new Float32Array(src.length);r=clamp(Math.round(r),1,20);for(let pass=0;pass<2;pass++){for(let y=0;y<h;y++)for(let x=0;x<w;x++){let s=0,n=0;for(let k=-r;k<=r;k++){const xx=pass?x:clamp(x+k,0,w-1),yy=pass?clamp(y+k,0,h-1):y;s+=a[yy*w+xx];n++}b[y*w+x]=s/n}[a,b]=[b,a]}return a}
const sample=(a,w,h,x,y)=>a[clamp(Math.round(y),0,h-1)*w+clamp(Math.round(x),0,w-1)];
function makeCanvas(w,h){const c=document.createElement('canvas');c.width=w;c.height=h;const o=c.getContext('2d',{willReadFrequently:true});o.fillStyle='#fff';o.fillRect(0,0,w,h);o.strokeStyle=o.fillStyle='#000';o.lineCap='round';o.lineJoin='round';return{c,o}}
...
function monoField(im){
  const w=im.width,h=im.height,g=gray(im),soft=blur(g,w,h,4),{c,o}=makeCanvas(w,h);
  const lineCount=clamp(Math.round(+controls.amount.value),25,180);
  const amplitude=clamp(+controls.warp.value,0,100)/100;
  const thickness=clamp(+controls.turb.value,0,100)/100;
  const spacing=w/lineCount,step=Math.max(1,h/500),gradientStep=Math.max(2,w/180);
  for(let line=0;line<lineCount;line++){
    const base=(line+.5)*spacing;
    let drift=0,velocity=0;
    o.beginPath();
    for(let y=0;y<=h;y+=step){
      const sx=clamp(base+drift,0,w-1);
      const tone=sample(g,w,h,sx,y);
      const left=sample(soft,w,h,sx-gradientStep,y),right=sample(soft,w,h,sx+gradientStep,y);
      const gx=(right-left)/(gradientStep*2);
      const darkness=1-tone;
      velocity=velocity*.88-gx*w*.34*amplitude;
      drift=drift*.992+velocity*.12+(darkness-.5)*spacing*.018*amplitude;
      drift=clamp(drift,-spacing*2.8,spacing*2.8);
      const x=clamp(base+drift,0,w);
      if(y===0)o.moveTo(x,y);else o.lineTo(x,y);
    }
    const centerTone=1-sample(g,w,h,base,h*.5);
    o.lineWidth=Math.max(.55,(w/900)+(centerTone*(.35+thickness*2.4)*w/900));
    o.stroke();
  }
  return finish(c);
}
function ascii(im,custom=false){const{c,o}=makeCanvas(im.width,im.height),g=gray(im),w=im.width,h=im.height,size=clamp(Math.round(+controls.amount.value),8,40),palette=custom?((($('#customText').value||'DITHER').trim()||'DITHER')):' .:-=+*#%@';o.textAlign='center';o.textBaseline='middle';o.font=`${size}px ui-monospace,SFMono-Regular,Menlo,monospace`;const cellX=Math.max(5,size*.62),cellY=Math.max(7,size*1.05);for(let y=cellY/2;y<h;y+=cellY)for(let x=cellX/2;x<w;x+=cellX){const tone=sample(g,w,h,x,y),idx=clamp(Math.floor((1-tone)*(palette.length-1)),0,palette.length-1);o.fillText(palette[idx]||' ',x,y)}return finish(c)}
const isField=()=>state.mode==='fieldMono';
const isAscii=()=>state.mode==='asciiClassic'||state.mode==='asciiCustom';
function render(){if(!state.image)return;try{const{width,height}=target(state.image);canvas.width=width;canvas.height=height;ctx.fillStyle='#fff';ctx.fillRect(0,0,width,height);ctx.drawImage(state.image,0,0,width,height);let im=ctx.getImageData(0,0,width,height);switch(state.mode){case'floyd':case'atkinson':case'stucki':case'sierra':case'riemersma':diffusion(im,state.mode);break;case'bayer':ordered(im,b4);break;case'ordered8':ordered(im,b8);break;case'bluenoise':noisy(im);break;case'grain':noisy(im,true);break;case'fieldMono':im=monoField(im);break;case'asciiClassic':im=ascii(im,false);break;case'asciiCustom':im=ascii(im,true);break;default:threshold(im)}forceBW(im.data);if(state.inverted)invert(im.data);ctx.putImageData(im,0,0);emptyState.hidden=true;sizeBadge.textContent=`${width} × ${height}`;downloadBtn.disabled=false}catch(error){console.error('Render failed',error);emptyState.hidden=false;emptyState.querySelector('strong').textContent='Erreur de rendu';emptyState.querySelector('span').textContent=error.message}}
...
function groups(){
  $('#basicControls').hidden=isField()||isAscii();
  $('#fieldControls').hidden=!isField();
  $('#asciiControls').hidden=!isAscii();
  $('#customTextWrap').hidden=state.mode!=='asciiCustom';
}
...
modes.forEach(b=>b.addEventListener('click',()=>{state.mode=b.dataset.mode;modes.forEach(x=>x.classList.toggle('active',x===b));if(isField()){controls.amount.value=92;controls.warp.value=72;controls.turb.value=55;controls.signalContrast.value=44;controls.brightness.value=-6}else if(isAscii()){controls.amount.value=14;controls.signalContrast.value=35;controls.brightness.value=0}groups();labels();schedule()}));
Object.values(controls).forEach(c=>c&&c.addEventListener('input',()=>{labels();schedule()}));
$('#customText').addEventListener('input',schedule);
